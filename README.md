...
## 🏃 新サービス: Athletics Knowledge Service

海外の陸上競技向けニッチ専門知識・ニュースを提供するマネタイズ用パッケージです。

### 起動手順

1. **バックエンド API (FastAPI)**

   ```bash
   cd services/athletics_knowledge
   pip install -r requirements.txt
   uvicorn api:app --reload
   ```

   → `http://127.0.0.1:8000/articles?topic=marathon` 等でアクセスできます。

2. **フロントエンド**
   `services/athletics_knowledge/frontend/index.html` をブラウザで開くか、Live Server等でプレビューしてください。

---

## 📚 第2弾新サービス: AI Academic Paper Reviewer

海外の研究者・学生向けに、arXivの最新論文を検索し、一流教授レベルの「6段階査読」と「Zotero連携」を自動化するSaaSパッケージです。
...
1. **バックエンド API (FastAPI)**
   ...
   ```bash
   cd services/paper_reviewer
   pip install -r requirements.txt
   uvicorn api:app --port 8001 --reload
   ```

   → `http://127.0.0.1:8001/search?query=AI` 等でAPIが動作します。

2. **フロントエンド**
   `services/paper_reviewer/frontend/index.html` をブラウザやLive Serverで開いてください。クリーンなUIから論文検索と査読生成が行えます。
