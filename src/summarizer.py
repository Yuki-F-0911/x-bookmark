...
def _summarize_chunk(
    ...
    prompt = f"""以下のXブックマーク一覧を分析し、**起業家やエンジニア向けの「生成AIとSaaS最新トレンド」**というテーマの特化型ニュースレターの原稿素材として抽出してください。

各ブックマークについて以下の3点を判定してJSON配列で返してください:
1. **importance**: "high"（AI・SaaSの重要ニュース・実用的なノウハウ）/ "normal"（一般的な情報）/ "low"（対象外・単なる感想）
2. **category**: 「{categories_str}」のいずれかに分類
3. **summary**: その情報が読者（起業家・エンジニア）にとって「なぜ重要か（示唆・インサイト）」を含めた2〜3文の専門的な要約。"low"の場合は "（内容なし）" としてください。

JSONのみで返してください（説明不要）:
[{{"id":"ID","importance":"high/normal/low","category":"カテゴリ","summary":"要約・インサイト"}}]
...
Web検索結果から読み取れる補足情報・背景・最新動向を抽出し、**起業家やエンジニアにとって有益な深いインサイトや具体例**として日本語で2〜3文にまとめてください。
