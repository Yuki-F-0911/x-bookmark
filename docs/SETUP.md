...
## ⑧ Athletics Knowledge Service のスケジューリング（オプション）

陸上競技トピックの自動取得を毎日実行するには、以下のコマンドをPowerShell（管理者権限）で実行します。

```powershell
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
cd C:\Users\yuki0\x-bookmark-digest
python scripts/naruse/daily_business_themes_pipeline.py --setup
```

出力されたスクリプトをPowerShellに貼り付けて実行してください。これにより、`AthleticsKnowledgeDaily` のタスクが登録されます。
